...
const inputClass = 'w-full outline-none px-8 bg-[#F7FAFC] border border-[#0000005a]  rounded-lg py-2'
const iconClass = 'w-10 rounded-full py-2.5 px-2.5 active:bg-[#FF7834] hover:no-underline flex flex-col items-center';
const counter = 'absolute -top-5 rounded-full -left-3 mr-4 border text-[10px] sm:px-2 px-1.5 py-[2px] bg-[#BD3826] border-[#BD3826] text-white sm:py-1'
    ...
    <div className="border-b-[1px] flex w-full p-[1.5rem] text-black bg-white pb-[1rem] fixed z-10">
        ...
        <div className="flex flex-col items-end justify-end w-1/3">
            <div className="flex items-center justify-end gap-5 py-1 rounded-full">
                    <Link to={'/internal/cart'} 
                        className={`${
                        location.pathname === '/internal/cart' ? 'border-[#FF7834] bg-[#FF7834]' : 'border bg-[#E6ECEB] w-10'
                        } ${iconClass}`}
                    >
                        <img src={pathname=== '/internal/cart' ? cart2 : cart} alt="" />
                    </Link>
                    <Link to={'/internal/notification'} 
                        className={`${
                        location.pathname === '/internal/notification' ? 'border-[#FF7834] bg-[#FF7834]' : 'border bg-[#E6ECEB] w-10'
                        } ${iconClass}`}
                    >
                        <div className="relative">
                                <img src={pathname === '/internal/notification' ? bell2 : bell} alt="" />
